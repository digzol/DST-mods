    ...
    if not inst:HasTag("burnt") then 
        ...
        if inst.lightson then
            ...
            if inst._window ~= nil then
                inst._window.AnimState:PlayAnimation("hit")
                inst._window.AnimState:PushAnimation("idle")
            end
            if inst._windowsnow ~= nil then
                inst._windowsnow.AnimState:PlayAnimation("hit")
                inst._windowsnow.AnimState:PushAnimation("idle")
            end
        else
...
local function MakeWindow()
    ...
    inst.AnimState:SetBank("pig_townhouse5")
...
return Prefab("hamlet_house5", fn, assets, prefabs),
    MakePlacer("hamlet_house5_placer", "pig_townhouse5", "pig_townhouse5_beige_build", "idle")
